...
def get_model(path,verbose=False):
    ...
    list_of_optim = glob.glob(os.path.join(path,'model*opt_state.pkl'))
    if list_of_files:
        # print(list_of_files)
        ...
        optim_fn = max(list_of_optim, key=os.path.getctime)
        print('Setting last optimization parameters : {}'.format(optim_fn))
        with open(optim_fn,'rb') as f:
...
def runNN(train_df,valid_df,epochs_per_set):
    ...
    weights_dir = os.path.dirname("../weights/state/ep{0:03d}/model.optim/".format(int(epochs_per_set)))
    ...
    # save optimization state at the end of epochs
    model_opt_state_fn = os.path.join(weights_dir,'model.set{0:03d}.epochs{1:04d}.opt_state.pkl'.format(set_nb,epochs_running+epochs_per_set))
    with open(model_opt_state_fn,'wb') as f:
