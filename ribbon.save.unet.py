...
def unet_101_incep(input_shape, conv_size=(5,5),pool_size=(2,2), n_labels=1,
                  initial_learning_rate=0.00001, deconvolution=False, bias=False):
    """
    Builds the 2D UNet Keras model.
    :param input_shape: Shape of the input data (n_chanels, x_size, y_size, z_size). 
    ...
    conv7a = Conv2D(70, (1,1), strides=pool_size, activation='relu', padding='same')(conv6)
    drop7a = Dropout(0.5)(conv7a)
    
    conv7b = Conv2D(70, (1,1), strides=pool_size, activation='relu', padding='same')(conv6)
    conv7b = Conv2D(70, (3,3), activation='relu', padding='same')(conv7b)
    drop7b = Dropout(0.5)(conv7b)
    
    conv7c = Conv2D(70, (1,1), strides=pool_size, activation='relu', padding='same')(conv6)
    conv7c = Conv2D(70, (5,5), activation='relu', padding='same')(conv7c)
    drop7c = Dropout(0.5)(conv7c)
...
def unet_103_incep(input_shape, conv_size=(5,5),pool_size=(4,4), n_labels=1,
                  initial_learning_rate=0.00001, deconvolution=False):
    """
    Builds the 2D UNet Keras model.
    :param input_shape: Shape of the input data (n_chanels, x_size, y_size, z_size). 
...
model = unet_101_incep(input_shape=input_shape, conv_size=(5,5),pool_size=(2,2), n_labels=2, bias=False)
json_string = model.to_json()
fn = "../model/model.unet.v101_incep.json"
print('Writing to file, model : {}'.format(fn))
with open(fn, 'w') as outfile:
    json.dump(json_string, outfile)
