...
def gen_tiles_random(img_fn,segment_fn,nb_tiles=1,tile_width=2560,verbose=False,drop_rate=50):
...
def runNN(train_df,valid_df,model_version,epochs_per_set,drop_rate):
    ...
    weights_dir = os.path.dirname("/home/rpizarro/histo/weights/drop/rate_{0:03d}/".format(drop_rate))
    set_nb,epochs_running=get_set_nb(weights_dir,epochs_per_set)
    print('This is set {} : epochs previously completed {} : epochs in this set {} : droprate {}'.format(set_nb,epochs_running,epochs_per_set,drop_rate))
    ...
    performance = History()
    set_path=os.path.join(weights_dir,'set{0:03d}'.format(set_nb),'model.r{0:03d}.set{1:03d}.'.format(drop_rate,set_nb)+'{epoch:04d}.valJacIdx{val_jaccard_index:0.3f}.h5')
    ...
    model.fit_generator(fileGenerator(train_df,nb_step=nb_step,verbose=False,input_size=input_size,output_size=output_size,drop_rate=drop_rate), steps_per_epoch=steps_per_epoch, epochs=epochs_per_set, verbose=1,
            validation_data=fileGenerator(valid_df,nb_step=1,verbose=True,input_size=input_size,output_size=output_size,drop_rate=0),validation_steps=1,callbacks=[performance,checkpointer])

    # save the weights at the end of epochs
    model_FINAL_fn = os.path.join(weights_dir,'model.r{0:03d}.set{1:03d}.epochs{2:04d}.FINAL.h5'.format(drop_rate,set_nb,epochs_running+epochs_per_set))
...
def fileGenerator(df,nb_step=1,verbose=True,input_size=(2560,2560,1),output_size=(2560,2560,2),drop_rate=50):
    ...
    while True:
        while n < nb_step:
            try:
                ...
                tiles,seg = gen_tiles_random(slice_fn,segment_fn,nb_step,tile_width,verbose=verbose,drop_rate=drop_rate)
...
# input to : python ribbon.train_unet.model.py 101(_drop) 10(100) 25
model_version = sys.argv[1]
epochs_per_set = int(sys.argv[2])
drop_rate = int(sys.argv[3])
...
runNN(train_df,valid_df,model_version,epochs_per_set,drop_rate)
